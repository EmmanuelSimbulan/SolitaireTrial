        ...
        while (!gameOver) {
            ...
            for (ArrayDeque<Card> column : tableau) {
                if (!column.isEmpty()) {
                    ...
                    // I think you need to fix this line of code since this isn't working also it does not move to foundation and the next card that is c;psed is not open to faceup card
			...
			// Missing the try moving cards from tableau to tableau
			// Cards must be placed in descending order in the tableau, starting from King down to Ace.
			// When moving cards, they must interleave in colors (red and black), so a red card can only be placed on top of a black card and vice versa. 
